        ...
        const fetchData = async () => {
            try {
                ...
                const addressResponse = await axios.get(
                    'http://localhost:3000/api/v2/user/addresses',
                    {
                        params: { email: email },
                    }
                );
                ...
                const cartResponse = await axios.get(
                    'http://localhost:3000/api/v2/product/cartproducts',
                    {
                        params: { email: email },
                    }
                );
    ...
    const handlePlaceOrder = async (paymentType = 'cod', paypalOrderData = null) => {
        try {
            ...
            const response = await axios.post(
                'http://localhost:3000/api/v2/orders/place-order',
                payload
            );
                        ...
                        {paymentMethod === 'paypal' && (
                            <div className='mt-4' style={{ maxWidth: '500px' }}>
                                <PayPalScriptProvider
                                    options={{
                                        'client-id': "AWMZ1WFkCgKVIw4WjCl8evJuVIm2lqhfhqw6_u7rTdTdEOYtoUg5Xi708SfmMT2bi8cZqPgLTfoYIxxJ",
                                    }}
                                >
                                    <PayPalButtons
